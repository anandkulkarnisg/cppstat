...
while [ "$retStatus" = "True" ]; do
  pythonResult=$(python3 $HOME/temp/winsorize.py $retVal|grep -i "after : " | awk -F":" '{print $2}'|sed "s/ //g")
  cppWinsorResult=$($HOME/git/cppstat/Example1.out $retVal|grep -i "after " | awk -F":" '{print $2}'|sed "s/ //g")
